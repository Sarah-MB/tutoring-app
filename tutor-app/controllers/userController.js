...
exports.createSubject = (req, res, next) =>{
  ...
  .then( result =>{
    if(result){
      ...
      res
      .status(200).json({ statsu: true, message: "subject created successfully", newSubject })
...
exports.createCategory = (req, res, next)=>{
  ...
  .then( result =>{
    if(result){
      ...
      res
      .status(200).send({ message: "category created successfully", newCategory })
    }).catch(err => console.log(err))
...
exports.makeTutorAdmin = (req, res, next) =>{
  ...
  .then(result =>{
    if(!result || result.role != 'tutor'){
      ...
      User.findByIdAndUpdate(tutorId, {role: 'admin' },{ new: true, useFindAndModify: false } 
      )
      .then( tutor =>{
        tutor.save()
        return res
